...
let cookie = '', res = '', shareCodes = [], homePageInfo = '', jxToken = '', UserName = ''
let shareCodesSelf = [], shareCodesHW = []
...
!(async () => {
  ...
  for (let [index, value] of cookiesArr.entries()) {
    ...
    let food = 0, petid = '', coins = 0, petNum = 0, petids = []
    try {
    ...
    // 助农
    let tasks = await api('GetUserTaskStatusList', 'bizCode,dateType,jxpp_wxapp_type,showAreaTaskFlag,source', {dateType: '2', showAreaTaskFlag: 0, jxpp_wxapp_type: 7}, true)
    await wait(2000)
...
async function api(fn, stk, params = {}, temporary = false) {
  let url
...
async function makeShareCodes(code) {
  try {
    ...
    let pin = decodeURIComponent(cookie.match(/pt_pin=([^;]*)/)[1]);
