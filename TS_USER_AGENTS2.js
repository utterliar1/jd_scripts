...
let fingerprint, token = '', enCryptMethodJD
...
function exceptCookie(filename = 'x.ts') {
  let except = []
...
async function getshareCodeHW(key) {
  let shareCodeHW = []
  for (let i = 0; i < 5; i++) {
    try {
      let {data} = await axios.get('https://api.jdsharecode.xyz/api/HW_CODES')
...
async function getShareCodePool(key, num) {
  ...
  for (let i = 0; i < 2; i++) {
    try {
      let {data} = await axios.get(`https://api.jdsharecode.xyz/api/${key}/${num}`)
      shareCode = data.data || []
...
/*async function wechat_app_msg(title, content, user) {
