 ...
 */
 ...
 var {getRandomNumberByRange, requireConfig, wait} = require("./TS_USER_AGENTS");
 
 
 let cookie = '', res = '', UserName, index
 let UA = `jdapp;JD4iPhone/167724 (iPhone; iOS ${getRandomNumberByRange(12, 16)}.${getRandomNumberByRange(0, 4)}; Scale/3.00)`
  ...
  // process.env.deleteCart = 'true'
   if (process.env.deleteCart === 'true') {
     ...
     for (let i = 0; i < cookiesArr.length; i++) {
       ...
       let areaId = res.areaId, traceId = res.traceId, delCount = 0, cartCount = parseInt(res.cartJson.num), venderCart = res.cart.venderCart
       ...
       if (cartCount > 0) {
         for (let vender of venderCart) {
           for (let sortedItem of vender.sortedItems) {
             let pid = sortedItem.polyItem?.promotion?.pid, postBody = ''
 
             for (let p of sortedItem.polyItem.products) {
               let commlist = p.mainSku.id, name = p.mainSku.name, skuUuid = p.skuUuid
