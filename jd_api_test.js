 ...
 */
 ...
 // var {format} = require("./sendNotify");
 var {format} = require("date-fns");
 var {getRandomNumberByRange, wait} = require("./TS_USER_AGENTS");
 
 !(async () => {
   console.log(`\n==================脚本执行- 北京时间(UTC+8)：${format(Date.now(), 'yyyy-MM-dd HH:mm:ss')}\n\n`)
 
   let cars = ['bean', 'farm', 'health', 'jxfactory', 'pet']
   let db = cars[Math.floor(Math.random() * cars.length)]
   let num = getRandomNumberByRange(5, 20)
